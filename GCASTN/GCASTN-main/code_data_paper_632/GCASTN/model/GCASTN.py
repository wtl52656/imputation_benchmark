...
class DspatialAttentionGCN(nn.Module):
    def __init__(self, sym_norm_Adj_matrix, in_channels, out_channels, dropout=.0):
        super(spatialAttentionGCN, self).__init__()
        self.sym_norm_Adj_matrix = sym_norm_Adj_matrix  # (N, N)
        self.in_channels = in_channels
...
class DspatialAttentionScaledGCN(nn.Module):
    def __init__(self, sym_norm_Adj_matrix, in_channels, out_channels, dropout=.0):
        super(DspatialAttentionScaledGCN, self).__init__()
        self.sym_norm_Adj_matrix = sym_norm_Adj_matrix  # (N, N)
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.Theta = nn.Linear(in_channels, out_channels, bias=False)
...
class SublayerConnection(nn.Module):
    ...
    def forward(self, x,delta, sublayer):
        '''
...
class decoderSublayerConnection(nn.Module):
    '''
    A residual connection followed by a layer norm
    '''
    def __init__(self, size, dropout, residual_connection, use_LayerNorm):
        super(decoderSublayerConnection, self).__init__()
        self.residual_connection = residual_connection
        self.use_LayerNorm = use_LayerNorm
        self.dropout = nn.Dropout(dropout)
        if self.use_LayerNorm:
...
class DPositionWiseGCNFeedForward(nn.Module):
    def __init__(self, gcn, dropout=.0):
        super(DPositionWiseGCNFeedForward, self).__init__()
        self.gcn = gcn
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, x,delta):
        ...
        :return: (B, N, T, F_out)
        '''
        return self.dropout(F.relu(self.gcn(x,delta)))


class MAAttention(nn.Module):
    def __init__(self, nb_head, d_model,dropout=.0,input_size=1):
        super(MAAttention, self).__init__()
        self.d_model = d_model
        self.init_score = torch.nn.Linear(1,d_model)
        self.out_score = torch.nn.Linear(d_model,1)
        self.w_dg_h = torch.nn.Linear(input_size, d_model)
        self.decay_att = MultiHeadAttention(nb_head, d_model,dropout)

    def forward(self,query, key, value,delta, mask=None, dropout=None):
        '''
        ...
        '''
        batch, N, h, T2, d_k = value.size()
        scores = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(d_k)  # scores: (batch, N, h, T1, T2)

        _,de_attn = self.decay_att(self.w_dg_h(delta),self.w_dg_h(delta),self.w_dg_h(delta))
        #gamma_h = torch.exp(-1 * torch.nn.functional.relu(self.w_dg_h(delta)))


        if mask is not None:
            scores = scores.masked_fill_(mask == 0, -1e9)  # -1e9 means attention scores=0
        #scores_h = self.init_score(scores.unsqueeze(-1))
        #gamma_h = torch.exp(-1 * torch.nn.functional.relu(self.w_dg_h(delta))).unsqueeze(2).repeat(1,1,h,1,1).unsqueeze(4).repeat(1,1,1,1,T2,1)
        #scores_h = scores_h * gamma_h
        #scores = self.out_score(scores_h).squeeze(-1)
        scores = de_attn * scores
...
class MAMultiHeadAttention(nn.Module):
    def __init__(self, nb_head, d_model, dropout=.0):
        super(MAMultiHeadAttention, self).__init__()
    ...
    def forward(self, query, key, value,delta, mask=None):
        '''
...
class MAMultiHeadAttentionAwareTemporalContex_q1d_k1d(nn.Module):  # 1d conv on query, 1d conv on key
    def __init__(self, nb_head, d_model, num_of_weeks, num_of_days, num_of_hours, points_per_hour, kernel_size=3,delta_input_size=1, dropout=.0):

        super(MAMultiHeadAttentionAwareTemporalContex_q1d_k1d, self).__init__()
        assert d_model % nb_head == 0
        self.d_k = d_model // nb_head
        self.h = nb_head
        self.linears = clones(nn.Linear(d_model, d_model), 2)  # 2 linear layers: 1  for W^V, 1 for W^O
        self.padding = (kernel_size - 1)//2
        self.w_dg_h = torch.nn.Linear(delta_input_size, d_model)
        ...
        self.attention = MAAttention( nb_head, d_model,dropout)


    def forward(self, query, key, value, delta,mask=None, query_multi_segment=False, key_multi_segment=False):
        '''
        :param query: (batch, N, T, d_model)
        :param key: (batch, N, T, d_model)
        :param value: (batch, N, T, d_model)
        ...
        # apply attention on all the projected vectors in batch
        x, self.attn = self.attention(query, key, value,delta, mask=mask, dropout=self.dropout)
        # x:(batch, N, h, T1, d_k)
        # attn:(batch, N, h, T1, T2)

        x = x.transpose(2, 3).contiguous()  # (batch, N, T1, h, d_k)
        x = x.view(nbatches, N, -1, self.h * self.d_k)  # (batch, N, T1, d_model)
        return self.linears[-1](x) #* gamma_h
...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, src,src_mask, trg,timestemps,delta):
        '''
        src:  (batch_size, N, T_in, F_in)
        src_mask:  (batch_size, N, T_in, F_in)
        trg: (batch, N, T_out, F_out)
        delta: (batch_size, N, T_in, F_in)
        '''
        encoder_output = self.encode(src,src_mask,timestemps,delta)  # (batch_size, N, T_in, d_model)

        return self.decode(trg, encoder_output)

    def encode(self, src, mask,timestemps,delta):
        '''
        src: (batch_size, N, T_in, F_in)
        delta: (batch_size, N, T_in, F_in)
        '''
        #new_src = torch.cat(dim=-1, tensors=[src, mask])
        #print("new_src.size()",new_src.size())
        h = self.src_embed(src)
        #h = self.grud(src,mask,timestemps,delta)
        #print("h.size()",h.size())
        encoder_output  = self.encoder(h,delta)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x,delta):
        ...
        self.feed_forward_gcn(x,delta)

        if self.residual_connection or self.use_LayerNorm:
            x = self.sublayer[0](x,delta, lambda x: self.self_attn(x, x, x,delta,query_multi_segment=True, key_multi_segment=True))
            return self.sublayer[1](x,delta, lambda x: self.feed_forward_gcn(x,delta))
        else:
            x = self.self_attn(x, x, x, delta,query_multi_segment=True, key_multi_segment=True)
            return self.feed_forward_gcn(x,delta)
...
class DecoderLayer(nn.Module):
    def __init__(self, size, self_attn, src_attn, gcn, dropout, residual_connection=True, use_LayerNorm=True):
        ...
        self.use_LayerNorm = use_LayerNorm
        if residual_connection or use_LayerNorm:
            self.sublayer = clones(decoderSublayerConnection(size, dropout, residual_connection, use_LayerNorm), 3)
...
def make_model(DEVICE, num_layers, encoder_input_size, decoder_output_size, d_model, adj_mx, nb_head, num_of_weeks,
    ...
    if ScaledSAt:  # employ spatial self attention
        e_position_wise_gcn = DPositionWiseGCNFeedForward(DspatialAttentionScaledGCN(norm_Adj_matrix, d_model, d_model), dropout=dropout)
        d_position_wise_gcn = PositionWiseGCNFeedForward(spatialAttentionScaledGCN(norm_Adj_matrix, d_model, d_model), dropout=dropout)
    else:  # 不带attention
    ...
    if aware_temporal_context:  # employ temporal trend-aware attention
        attn_ss = MAMultiHeadAttentionAwareTemporalContex_q1d_k1d(nb_head, d_model, num_of_weeks, num_of_days, num_of_hours, num_for_predict, kernel_size, dropout=dropout)  # encoder的trend-aware attention用一维卷积
    ...
    encoderLayer = EncoderLayer(d_model, attn_ss, e_position_wise_gcn, dropout, residual_connection=residual_connection, use_LayerNorm=use_LayerNorm)

    encoder = Encoder(encoderLayer, num_layers)
    #cde_module = NeuralCDE(input_channels=encoder_input_size+1, hidden_channels=d_model, output_channels=16)

    decoderLayer = DecoderLayer(d_model, att_tt, attn_st, d_position_wise_gcn, dropout, residual_connection=residual_connection, use_LayerNorm=use_LayerNorm)
