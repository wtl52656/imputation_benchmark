...
class CSDI_base(nn.Module):
    def __init__(self, target_dim, config):
        ...
        self.alpha_torch = torch.tensor(self.alpha).float().cuda().unsqueeze(1).unsqueeze(1)

    def time_embedding(self, pos, d_model=128):
        pe = torch.zeros(pos.shape[0], pos.shape[1], d_model).cuda()
        position = pos.unsqueeze(2)
        div_term = 1 / torch.pow(
            10000.0, torch.arange(0, d_model, 2).cuda() / d_model
        )
    ...
    def get_side_info(self, observed_tp, cond_mask):
        ...
        feature_embed = self.embed_layer(
            torch.arange(self.target_dim).cuda()
        )  # (K,emb)
    ...
    def calc_loss(
        ...
        if is_train != 1:  # for validation
            t = (torch.ones(B) * set_t).long().cuda()
        else:
            t = torch.randint(0, self.num_steps, [B]).cuda()    #duffusion随机步长
    ...
    def impute(self, observed_data, cond_mask, side_info, n_samples):
        ...
        imputed_samples = torch.zeros(B, n_samples, K, L).cuda()
        ...
        for i in range(n_samples):
            ...
            for t in range(self.num_steps - 1, -1, -1):
                ...
                predicted = self.diffmodel(diff_input, side_info, torch.tensor([t]).cuda())
...
class CSDI_Traffic(CSDI_base):
    def __init__(self, config,  target_dim):
        super(CSDI_Traffic, self).__init__(target_dim, config)

    def process_data(self, batch,is_train=1):
        observed_data = batch["observed_data"].cuda().float()
        observed_mask = batch["observed_mask"].cuda().float()
        observed_tp = batch["timepoints"].cuda().float()
        gt_mask = batch["gt_mask"].cuda().float()
